...
class PayPalGateway extends AbstractGateway {
	...
	public function get_payment_method_data( $context ) {
		$data = [
			'funding'              => array_values( array_filter( [ 'paypal', 'paylater', 'card', 'venmo' ], function ( $source ) {
				if ( $source === 'paypal' ) {
					return true;
				}
		...
		if ( $context->is_product() ) {
			...
			$data['funding']                 = array_filter( [ 'paypal', 'paylater', 'card' ], function ( $type ) use ( $settings ) {
	...
	protected function get_payment_section_options( $type = 'paypal' ) {
		...
		if ( $type === 'paypal' ) {
			// If PayPal is enabled, it' always enbled on checkout
