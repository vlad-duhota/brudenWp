...
class PayLaterMessageSettings extends AbstractSettings {
	...
	public function handle_after_order_total() {
		if ( wc_string_to_bool( $this->get_option( 'checkout_enabled' ) ) && 'shop_table' === $this->get_option( 'checkout_location' ) ) {
			?>
	...
	public function handle_cart_totals() {
		if ( wc_string_to_bool( $this->get_option( 'cart_enabled' ) ) && 'shop_table' === $this->get_option( 'cart_location' ) ) {
			?>
            <tr class="wc-ppcp-paylater-msg__container" style="display: none">
                <td colspan="2">
