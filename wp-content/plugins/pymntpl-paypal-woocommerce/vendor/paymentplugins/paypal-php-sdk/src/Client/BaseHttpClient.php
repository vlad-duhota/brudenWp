...
abstract class BaseHttpClient extends AbstractClient {
	...
	public function __construct( $config = array() ) {
		...
		$this->client = new \GuzzleHttp\Client( array(
			...
			'verify'      => dirname( dirname( __DIR__ ) ) . '/ca-certificates.crt',
			'http_errors' => false
		) );
	}
	...
	public function request( $method, $path, $responseClass = null, $params = null, $options = array() ) {
		try {
			...
			$response = $this->client->request( strtoupper( $method ), $this->getRequestUrl( $path ), (array) $options );
			...
			if ( in_array( $statusCode, self::SUCCESS_RESPONSE_CODES ) ) {
				...
				$this->handleApiException( $statusCode, $this->decodeResponseBody( $response->getBody() ) );
			}
		} catch ( \GuzzleHttp\Exception\GuzzleException $e ) {
			// catch exception and return PayPal exception
